...
export const getDocumentsByFieldValues = async (
    ...
    try {
        ...
        fieldValuePairs.forEach(({ fieldName, fieldValue }: FieldValuePair): void => {
...
export const deleteDocumentsByFieldValues = async (
    ...
    try {
        ...
        if (transaction) {
            ...
            snapshot.docs.forEach((doc: FirebaseFirestore.QueryDocumentSnapshot): void => {
        ...
        const fieldValueString: string = fieldValuePairs
            .map(({ fieldName, fieldValue }: FieldValuePair): string => `${fieldName} == ${fieldValue}`)
            .join(" AND ");
        const errorMessage: string =
            error instanceof Error ? error.message : "Unknown error";
        throw new Error(
